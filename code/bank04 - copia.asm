...
Palette0_MTiles:
  ...
  .db $d0, $70, $2c, $2d ;mushroom left edge 19
  .db $d1, $71, $d2, $72 ;mushroom middle 1a
  .db $d3, $73, $d4, $74 ;mushroom right edge 1b
...
Palette1_MTiles:
  ...
  .db $75, $ba, $76, $43 ;mushroom stump top 4f
...
TerrainRenderBits:
      ...
      .db %00000011, %00011000 ;ceiling 2, floor 2
...
LevelBanks:
ProcessAreaData:
            ldx #$0b ;start at the end of area object buffer
			lda $fc
			sec
			sbc #$66
			sta tempA
ProcADLoop: stx ObjectOffset
			...
			beq ProcADLoop			;unconditional branch
+:          iny
...
CheckRear:  lda AreaObjectPageLoc    ;check to see if current page of level object is
            cmp tempA	             ;behind current page of renderer
...
SetBehind:  inc BehindAreaParserFlag ;turn on flag if object is behind renderer
			...
			;clc
NextAObj:   inc AreaDataOffset
			inc AreaDataOffset
		    ;lda #$00              ;reset page select
		    ;sta AreaObjectPageSel     ;increment buffer offset and move on
...
ProcLoopb:  dex                      ;decrement buffer offset
            bpl ProcADLoop1            ;and loopback unless exceeded buffer
...
Chk1stB:  ldx #$10                   ;load offset of 16 for special row 15
		  ...
		  if shufflelevels !=1
			  cmp #$fd
			  beq XD              ;if end of level, leave this routine
		  endif
...
ChkRow14: stx $07                    ;store whatever value we just loaded here
          ...
          sta $07
          lda #$34                   ;and load A with another value (Default 2e, special row objects)
          bne NormObj                ;unconditional branch
ChkRow13: cmp #$0d                   ;row 13?
          bne ChkSRows
          lda #$28                   ;if so, load offset with (Default: 22, smalls objects)
          sta $07
          iny                        ;get next byte
          lda (AreaData),y
...
NormObj:  sta $00                    ;store value here (branch for small objects and rows 13 and 14)
          ...
          cmp tempA        			 ;same page as the renderer, and if so, branchjmp BackColC
		  beq InitRear
...
;large objects (rows $00-$0b or 00-11, d6-d4 set) xxxxyyyy, aXXXhhhh (3 bits of ID = 8 objects)
      .dw VerticalPipe         	;00 warp pipes			0
      .dw AreaStyleObject		;01						1
      .dw RowOfBricks			;02						2
      .dw RowOfSolidBlocks		;03						3
      .dw RowOfCoins			;04						4
      .dw ColumnOfBricks		;05						5
      .dw ColumnOfSolidBlocks	;06						6
      .dw VerticalPipe         	;07 decoration pipes 	7

;objects for special row $0c or 12 xxxx1100, ahhhhXXX (fixed Y pos) (3 bits of ID = 8 objects)
      .dw Hole_Empty			;08						0
      .dw PulleyRopeObject		;09						1
      .dw Bridge_High			;0a						2
      .dw Bridge_Middle			;0b						3
      .dw Bridge_Low			;0c						4
      .dw Hole_Water			;0d						5
      .dw QuestionBlockRow_High	;0e						6
      .dw QuestionBlockRow_Low	;0f						7

;objects for special row $0f or 15 xxxx1111, ahhhhXXX (fixed y pos, except for expanded objects 5-7) (3 bits of ID = 8 objects)
      .dw EndlessRope			;10						0
      .dw BalancePlatRope		;11						1
      .dw CastleObject			;12						2
      .dw StaircaseObject		;13						3
      .dw ExitPipe				;14						4
      .dw UpsideDownWarpPipe	;15 					5
	  .dw UpsideDownPipe     	;16 					6
	  .dw CustomMTObject        ;17						7
      
;small objects (rows $00-$0b or 00-11, d6-d4 all clear)	xxxxyyyy, a000XXXX (4 bits of ID = 16 objects)
      .dw QuestionBlock     	;18 power-up			0
      .dw QuestionBlock     	;19 coin				1
      .dw HiddenCoin         	;1a hidden, coin		2
      .dw Hidden1UpBlock    	;1b hidden, 1-up		3
      .dw BrickWithPowerUp     	;1c brick, power-up		4
      .dw BrickWithVine     	;1d brick, vine			5
      .dw BrickWithStar     	;1e brick, star			6
      .dw BrickWithCoins    	;1f brick, coins		7
      .dw BrickWith1up	     	;20 brick, 1-up			8
      .dw WaterPipe				;21						9
      .dw EmptyBlock			;22						a
      .dw Jumpspring			;23						b
	  .dw HiddenMushRoom		;24 hidden, power-up	c
	  .dw HiddenStar			;25	hidden, star		d
	  .dw QuestionBlockStar		;26	star				e
	  .dw QuestionBlock1Up		;27	1-up				f

;objects for special row $0d or 13 (d6 set) xxxx1101, a0XXXXXX (7 bits of ID = 128 objects)
													; a1XXXbbb (for page skip)
      .dw IntroPipe				;28						0
      .dw FlagpoleObject		;29						1
      .dw AxeObj				;2a						2
      .dw ChainObj				;2b						3
      .dw CastleBridgeObj		;2c						4
      .dw ScrollLockObject_Warp	;2d						5
      .dw ScrollLockObject		;2e						6
      .dw ScrollLockObject		;2f dupl. of 2e			7
      .dw AreaFrenzy            ;30 flying cheep-cheeps 8
      .dw AreaFrenzy            ;31 bbills or swim fish 9
      .dw StopFrenzy            ;32 stop frenzy			a
      .dw LoopCmdE				;33						b
...
;object for special row $0e or 14 xxxx1110, ajssssbbb
      .dw AlterAreaAttributes	;34						?
...
;jump engine table for newly loaded enemy objects
      ...
      .dw InitLakitu
      .dw InitEnemyFrenzy  ;12 spiny
      .dw InitJumpGPTroopa ;13
      .dw InitEnemyFrenzy  ;flying cheep-cheeps zone
      .dw InitEnemyFrenzy  ;bowser's fire
      .dw InitEnemyFrenzy  ;fireworks (unstable)
      .dw InitEnemyFrenzy  ;bullet bills / underwater cheep-cheeps
      .dw EndFrenzy		   ;enf frenzy
      .dw InitKoopaShell	
      .dw NoInitCode	   ;null sprite
      .dw InitShortFirebar
      .dw InitShortFirebar
      .dw InitShortFirebar
      .dw InitShortFirebar
      ...
      .dw NoInitCode ;those 3 are long firebars (malfunctions)
      .dw NoInitCode

      .dw InitUDPiranhaPlant
      .dw InitBalPlatform
      .dw InitVertPlatform
      .dw LargeLiftUp
      .dw LargeLiftDown
      ...
      .dw PlatLiftDown
      .dw InitBowser
      .dw InitPowerUp   ;possibly dummy value
      .dw Setup_Vine

      .dw NoInitCode ;for objects $30-$36, flagpole flag object
      .dw NoInitCode ;star flag object
      .dw NoInitCode ;jumpspring object
      .dw NoInitCode ;bullet bill cannon var
      .dw NoInitCode ;warp zone
...
MetatileThingy1:
	...
	.db $54, $54, $54, $23, $25, $25, $26, $27, $28, $5f, $60, $6f, $54, $54, $2e, $2f
	...
	.db $91, $91, $92, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00
	.db $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00
	.db $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00
	
	.db $c0, $c1, $c2, $c3, $54, $c5, $c6, $c7, $00, $00, $00, $00, $00, $00, $00, $00
	.db $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00
	.db $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00	
	.db $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00, $00
