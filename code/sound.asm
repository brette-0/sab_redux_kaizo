...
_PlayPipeDownInj: jmp PlayPipeDownInj
_PlayFlagpoleSlide: jmp PlayFlagpoleSlide
...
Square1SfxHandler:
             ...
             lsr Square1SoundQueue
             bcs _PlayPipeDownInj    ;pipedown/injury
             lsr Square1SoundQueue
             bcs PlayFireballThrow   ;fireball throw
             lsr Square1SoundQueue
             bcs _PlayFlagpoleSlide  ;slide flagpole
...
ContinueSmackEnemy:
             ...
             cmp #Sfx_EnemySmackOr
             beq NormalContinueSwimStomp
             ldy Squ1_SfxLenCounter  ;check about halfway through
             cpy #%00001000
             bne SmSpc
             lda #%10100000          ;if we're at the about-halfway point, make the second tone
;stomp counter 1: #%10100000
             lda #%10100
             sec
             ...
             lda #%10011111
             bne SmTick
SmSpc:  lda #%10010000          ;this creates spaces in the sound, giving it its distinct noise
...
NormalContinueSwimStomp:
             ...
             lda #$9f
             bne SmTic1
SmSpc1: lda #$90                ;this creates spaces in the sound, giving it its distinct noise
SmTic1: sta SND_SQUARE1_REG
             lda #$00
             sta tempA
...
PlayTimerTick:
             lda #$09            ;load length of timer tick sound
...
ContinueBlast:
             ...
             lda #$18
SBlasJ: jmp BlstSJp             ;unconditional branch to load rest of reg contents
...
StopSquare2Sfx:
             jmp _StopSquare2Sfx
ExSfx2: rts
PlayTimerTick1: jmp PlayTimerTick
PlayCoinGrab2:  jmp PlayCoinGrab
Square2SfxHandler:
...
EL_LRegs: jmp LoadSqu2Regs         ;this is an unconditional branch outta here
PlayExtraLife:
...
NotECstlM: lda AreaMusicBuffer
             ...
             bne AllMus
WaterMus:  lda #$27              ;this value is used for water music and all other event music
...
_StopSquare2Sfx:
             pha
             lda SND_MASTERCTRL_REG
             and #%00010000          ;keep only the dmc bit
             ora #$0d                ;stop playing the sfx
             tax
             pla
             stx SND_MASTERCTRL_REG ; write #$d to lower nibble
             inx
             inx
             stx SND_MASTERCTRL_REG ; write #$f to lower nibble
             rts
...
MusicHeaderData:                    ;starts from 01 (or 00)
             .db DeathMusHdr-MHD           ;01 - event music
             .db GameOverMusHdr-MHD        ;02
             .db VictoryMusHdr-MHD            ;03
             .db WinCastleMusHdr-MHD        ;04
             .db GameOverMusHdr-MHD        ;05
             .db EndOfLevelMusHdr-MHD        ;06
             .db TimeRunningOutHdr-MHD        ;07
             .db SilenceHdr-MHD            ;08
             
             .db GroundLevelPart1Hdr-MHD   ;09 - area music
             .db WaterMusHdr-MHD            ;0a
             .db UndergroundMusHdr-MHD        ;0b
             .db CastleMusHdr-MHD            ;0c
             .db Star_CloudHdr-MHD            ;0d
             .db GroundLevelLeadInHdr-MHD    ;0e
             .db Star_CloudHdr-MHD            ;0f
             .db SilenceHdr-MHD            ;10
             
             .db GroundLevelLeadInHdr-MHD  ;12 - ground level music layout
             .db GroundLevelPart1Hdr-MHD, GroundLevelPart1Hdr-MHD                                                            ;13 (13-14)
             .db GroundLevelPart2AHdr-MHD, GroundLevelPart2BHdr-MHD, GroundLevelPart2AHdr-MHD, GroundLevelPart2CHdr-MHD     ;14    (15-18)
             .db GroundLevelPart2AHdr-MHD, GroundLevelPart2BHdr-MHD, GroundLevelPart2AHdr-MHD, GroundLevelPart2CHdr-MHD    ;15    (19-1c)
             .db GroundLevelPart3AHdr-MHD, GroundLevelPart3BHdr-MHD, GroundLevelPart3AHdr-MHD, GroundLevelLeadInHdr-MHD    ;16    (1d-20)
             .db GroundLevelPart1Hdr-MHD, GroundLevelPart1Hdr-MHD                                                            ;17    (21-22)
             .db GroundLevelPart4AHdr-MHD, GroundLevelPart4BHdr-MHD, GroundLevelPart4AHdr-MHD, GroundLevelPart4CHdr-MHD    ;18    (23-26)
             .db GroundLevelPart4AHdr-MHD, GroundLevelPart4BHdr-MHD, GroundLevelPart4AHdr-MHD, GroundLevelPart4CHdr-MHD    ;19    (27-2a)
             .db GroundLevelPart3AHdr-MHD, GroundLevelPart3BHdr-MHD, GroundLevelPart3AHdr-MHD, GroundLevelLeadInHdr-MHD    ;1a    (2b-2e)
             .db GroundLevelPart4AHdr-MHD, GroundLevelPart4BHdr-MHD, GroundLevelPart4AHdr-MHD, GroundLevelPart4CHdr-MHD    ;1b    (2f-32)
             
             .db PSwitchMusHdr-MHD            ;32
             
TimeRunningOutHdr:    .db $08, <TimeRunOutMusData, >TimeRunOutMusData, $27, $18        ;01
Star_CloudHdr:        .db $20, <Star_CloudMData, >Star_CloudMData, $2e, $1a, $40    ;02
EndOfLevelMusHdr:     .db $20, <WinLevelMusData, >WinLevelMusData, $3d, $21            ;03
ResidualHeaderData:   .db $20, $c4, $fc, $3f, $1d                                    ;04
UndergroundMusHdr:    .db $18, <UndergroundMusData, >UndergroundMusData, $00, $00    ;05
SilenceHdr:           .db $08, <SilenceData, >SilenceData, $00                        ;06
CastleMusHdr:         .db $00, <CastleMusData, >CastleMusData, $93, $62                ;07
VictoryMusHdr:        .db $10, <VictoryMusData, >VictoryMusData, $24, $14            ;08
GameOverMusHdr:       .db $18, <GameOverMusData, >GameOverMusData, $1e, $14            ;09
WaterMusHdr:          .db $08, <WaterMusData, >WaterMusData, $a0, $70, $68            ;0a
WinCastleMusHdr:      .db $08, <EndOfCastleMusData, >EndOfCastleMusData, $4c, $24    ;0b
GroundLevelPart1Hdr:  .db $18, <GroundM_P1Data, >GroundM_P1Data, $2d, $1c, $b8        ;0c
GroundLevelPart2AHdr: .db $18, <GroundM_P2AData, >GroundM_P2AData, $20, $12, $70    ;0d
GroundLevelPart2BHdr: .db $18, <GroundM_P2BData, >GroundM_P2BData, $1b, $10, $44    ;0e
GroundLevelPart2CHdr: .db $18, <GroundM_P2CData, >GroundM_P2CData, $11, $0a, $1c    ;0f
GroundLevelPart3AHdr: .db $18, <GroundM_P3AData, >GroundM_P3AData, $2d, $10, $58    ;10
GroundLevelPart3BHdr: .db $18, <GroundM_P3BData, >GroundM_P3BData, $14, $0d, $3f    ;11
GroundLevelLeadInHdr: .db $18, <GroundMLdInData, >GroundMLdInData, $15, $0d, $21    ;12
GroundLevelPart4AHdr: .db $18, <GroundM_P4AData, >GroundM_P4AData, $18, $10, $7a    ;13
GroundLevelPart4BHdr: .db $18, <GroundM_P4BData, >GroundM_P4BData, $19, $0f, $54    ;14
GroundLevelPart4CHdr: .db $18, <GroundM_P4CData, >GroundM_P4CData, $1e, $12, $2b    ;15
DeathMusHdr:          .db $18, <DeathMusData, >DeathMusData, $1e, $0f, $2d            ;16
PSwitchMusHdr:          .db $18, <PSwitch, >PSwitch, $44, $75                            ;17
...
PSwitch:
             .db $84, $3a, $04, $3a, $04, $82, $38, $3a, $84, $04, $3a, $04, $40, $04, $40, $04 ;SQ2
             .db $82, $3c, $40, $84, $04, $40, $04, $3a, $04, $3a, $04, $82, $38, $3a, $84, $04
             .db $3a, $04, $40, $04, $40, $04, $82, $3c, $40, $84, $04, $40, $04, $3e, $04, $3e
             .db $04, $82, $40, $3e, $84, $04, $3e, $04, $64, $04, $64, $04, $82, $42, $64, $84
             .db $04, $64, $04, $00
             
             .db $84, $32, $3e, $4a, $3e, $32, $3e, $4a, $3e, $36, $64, $4c, $64, $36, $64, $4c ;TRI  ;44
             .db $64, $32, $3e, $4a, $3e, $32, $3e, $4a, $3e, $36, $64, $4c, $64, $36, $64, $4c
             .db $64, $38, $46, $50, $46, $38, $46, $50, $46, $3c, $4a, $50, $4a, $3c, $4a, $50
             .db $4a
             
             .db $29, $05, $29, $05, $a6, $a8, $05, $29, $05, $2d, $05, $2d, $05, $aa, $ac, $05 ;SQ1  ;75
             .db $2d, $05, $29, $05, $29, $05, $a6, $a8, $05, $29, $05, $2d, $05, $2d, $05, $aa
             .db $ac, $05, $2d, $05, $2f, $05, $2f, $05, $ac, $ae, $05, $2f, $05, $33, $05, $33
             .db $05, $b0, $b2, $05, $33, $05 ;af
